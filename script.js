...
let score = 0;
const WIN_SCORE = 20; // Hearts needed to fill the meter
let player;
let hearts = [];
...
// Emoji Object (Hug / Kiss)
class Emoji {
    constructor(type) {
        this.type = type; // 'hug' or 'kiss'
    ...
    draw() {
        ctx.font = `${this.size}px "Apple Color Emoji", "Segoe UI Emoji", serif`;
...
function spawnEmoji() {
    if (Math.random() < 0.04) {
        const type = Math.random() > 0.5 ? 'hug' : 'kiss';
...
function triggerConfetti() {
    ...
    for (let i = 0; i < 50; i++) {
        emojis.push(new Emoji(Math.random() > 0.5 ? 'hug' : 'kiss'));
    }
}
